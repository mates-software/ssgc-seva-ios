...
class PreviewViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if SettingsContext.shared.previewIntersectionsIncludeUnnamedRoads {
            roadToggleButton = UIBarButtonItem(
                image: UIImage(named: "preview_toggle_on"), style: .plain, target: self,
                action: #selector(onRoadToggleButtonTouchUpInside))
            roadToggleButton.accessibilityLabel = GDLocalizedString(
                "preview.include_unnamed_roads.label.on")
            roadToggleButton.accessibilityHint = GDLocalizedString(
                "preview.include_unnamed_roads.hint.on")
        } else {
            roadToggleButton = UIBarButtonItem(
                image: UIImage(named: "preview_toggle_off"), style: .plain, target: self,
                action: #selector(onRoadToggleButtonTouchUpInside))
            roadToggleButton.accessibilityLabel = GDLocalizedString(
                "preview.include_unnamed_roads.label.off")
            roadToggleButton.accessibilityHint = GDLocalizedString(
                "preview.include_unnamed_roads.hint.off")
        }
    ...
    private func initializePreview(at locationDetail: LocationDetail) {
        ...
        let progress = LocationActionHandler.preview(locationDetail: locationDetail) {
            [weak self] (result) in
    ...
    private func onPreviewDidInitialize(
        _ result: Result<PreviewBehavior<IntersectionDecisionPoint>, LocationActionError>
    ) {
    ...
    private func configureToggleButton() {
        DispatchQueue.main.async { [weak self] in
            ...
            if SettingsContext.shared.previewIntersectionsIncludeUnnamedRoads {
                ...
                self.roadToggleButton.accessibilityLabel = GDLocalizedString(
                    "preview.include_unnamed_roads.label.on")
                self.roadToggleButton.accessibilityHint = GDLocalizedString(
                    "preview.include_unnamed_roads.hint.on")
            } else {
    ...
    private func configureContainerView(
        _ container: UIView, isHidden: Bool, navigationBarIsHidden: Bool,
        roadToggleItemIsHidden: Bool
    ) {
        DispatchQueue.main.async {
            // Update the state of the activity indicator view
            UIView.transition(
                with: self.view, duration: 1.0, options: .transitionCrossDissolve,
                animations: { [weak self] in
                    ...
                    self.navigationController?.setNavigationBarHidden(
                        navigationBarIsHidden, animated: true)
                },
                completion: { [weak self] (_) in
                    ...
                    UIAccessibility.post(
                        notification: UIAccessibility.Notification.layoutChanged, argument: argument
                    )
                })
        }
    }
    ...
    private func configureActivityIndicatorView(isHidden: Bool) {
        ...
        configureContainerView(
            activityIndicatorContainerView, isHidden: isHidden, navigationBarIsHidden: false,
            roadToggleItemIsHidden: !isHidden)
    }

    private func configureTutorialView(isHidden: Bool) {
        configureContainerView(
            tutorialContainerView, isHidden: isHidden, navigationBarIsHidden: isHidden == false,
            roadToggleItemIsHidden: !isHidden)
    }
    ...
    @objc func onRoadToggleButtonTouchUpInside() {
        ...
        GDATelemetry.track(
            "preview.include_unnamed_roads",
            with: [
                "value": "\(SettingsContext.shared.previewIntersectionsIncludeUnnamedRoads)",
                "context": "preview",
            ])
    ...
    private func continueUserAction(_ notification: Notification) {
        guard
            let userAction = notification.userInfo?[UserActionManager.Keys.userAction]
                as? UserAction
        else { return }
