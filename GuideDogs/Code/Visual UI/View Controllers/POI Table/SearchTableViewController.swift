...
private class SearchTableDataSource: StaticTableViewDataSource {
    ...
    init(
        header: String?, cells: [StaticTableViewCell],
        currentLocationActions: [UIAccessibilityCustomAction]
    ) {
...
class SearchTableViewController: BaseTableViewController {
    ...
    private var saveCurrentLocationAction: UIAccessibilityCustomAction {
        return UIAccessibilityCustomAction(name: GDLocalizedString("menu.events")) {
            [weak self] _ -> Bool in
            DispatchQueue.main.async { [weak self] in
                ...
                // Navegar a Recreation Activities
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        GDATelemetry.trackScreenView(
            title ?? "Select a Location", with: ["context": delegate?.usageLog ?? logContext])
    }
    ...
    private func updateTableView() {
        DispatchQueue.main.async { [weak self] in
            ...
            if self.parent is HomeViewController {
                ...
                self.tableViewDataSource = SearchTableDataSource(
                    header: nil, cells: self.moreLocations,
                    currentLocationActions: [self.saveCurrentLocationAction])
            } else {
                self.tableViewDataSource = StaticTableViewDataSource(
                    header: nil, cells: self.moreLocations)
            }
            ...
            do {
                ...
                case .share:
                    ...
                    if FirstUseExperience.didComplete(.share) {
                        ...
                        let firstUseAlert = ShareMarkerAlert.firstUseExperience(dismissHandler: {
                            [weak self] _ in
                            guard let `self` = self else {
                                return
                            }
